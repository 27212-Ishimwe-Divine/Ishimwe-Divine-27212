...
- GET /api/menu/available?available={true|false} — filter by availability
- GET /api/menu/search?name={name} — search by name
- POST /api/menu — add menu item (JSON body)
- PUT /api/menu/{id}/availability — toggle availability
- DELETE /api/menu/{id} — remove menu item

Run:
```powershell
mvn -f question3-restaurant-api spring-boot:run
```

Sample response (GET /api/menu): 200 OK
```json
[{"id":1,"name":"Bruschetta","description":"Grilled bread with tomatoes","price":6.5,"category":"Appetizer","available":true}]
```
